...
public class Shooter extends SubsystemBase {
  ...
  private final double kShooterkV = kWheelDiameter.in(Meters)*Math.PI/kVortexKv.in(RevolutionsPerSecond); 
  ...
  .withFeedforward(new SimpleMotorFeedforward(0, kShooterkV, 0)) // meters per second, not RPM
 ...
 private final FlyWheelConfig shooterConfig = new FlyWheelConfig(sparkSmartMotorController)
  ...
  .withUpperSoftLimit(RPM.of(6800)) // TODO: check limit?
  ...
  public Command RunShooterCommand() {
    return runOnce(
        () -> {
          ...
          shooter.setMeasurementVelocitySetpoint(FeetPerSecond.of(70.0));
        });
...
public Command StopShooterCommand() {
  return runOnce(
    () -> {
      shooter.setMeasurementVelocitySetpoint(FeetPerSecond.of(0.1));
